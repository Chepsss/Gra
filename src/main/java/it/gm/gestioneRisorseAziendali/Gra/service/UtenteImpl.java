...
public class UtenteImpl implements UtenteInterface {
	...
	public Utente updateUtente(Utente utente) {
		...
		try {
			...
			if(utente.getAbilitato() == true){
				utenteAggiornato.setAbilitato(true);
				}
				else  {
					utenteAggiornato.setAbilitato(false);
				}
	...
	@Override
	public List<Utente> getAllDaAbilitareDisab(boolean valore) {
		List<Utente> lista = new ArrayList<>();
		try {
			lista= repository.findAllByAbilitato(valore);
			return lista;
		}
		catch(Exception e){
			e.printStackTrace();
